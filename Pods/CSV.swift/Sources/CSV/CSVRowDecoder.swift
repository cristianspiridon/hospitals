...
open class CSVRowDecoder {
    ...
    open func decode<T: Decodable>(_ type: T.Type, from reader: CSVReader) throws -> T {
        let decoder = _CSVRowDecoder(referencing: reader, options: options)
...
private final class CSVKeyedDecodingContainer<K: CodingKey>: KeyedDecodingContainerProtocol {
    ...
    public var allKeys: [Key] {
        guard let headerRow = decoder.reader.headerRow else { return [] }
    ...
    private func value(for key: Key) throws -> String {
        ...
        if let index = key.intValue {
            return decoder.reader.currentRow![index]
        } else {
            return decoder.reader[key.stringValue]!
        }
    }

    private func _valueNotFound(_ type: Any.Type) -> DecodingError {
        let description = "Expected \(type) value but found null instead."
        return .valueNotFound(type, DecodingError.Context(codingPath: decoder.codingPath, debugDescription: description))
    }

    public func contains(_ key: Key) -> Bool {
        guard let row = decoder.reader.currentRow else { return false }
    ...
    public func decode(_ type: Bool.Type, forKey key: Key) throws -> Bool {
        ...
        guard let result = try decoder.unbox(value, as: Bool.self) else {
    ...
    public func decode(_ type: String.Type, forKey key: Key) throws -> String {
        ...
        guard let result = try decoder.unbox(value, as: String.self) else {
            throw _valueNotFound(type)
        }
        return result
    }
    ...
    public func decode(_ type: Double.Type, forKey key: Key) throws -> Double {
        ...
        guard let result = try decoder.unbox(value, as: Double.self) else {
            throw _valueNotFound(type)
        }
        return result
    }
    ...
    public func decode(_ type: Float.Type, forKey key: Key) throws -> Float {
        ...
        guard let result = try decoder.unbox(value, as: Float.self) else {
            throw _valueNotFound(type)
        }
        return result
    }
    ...
    public func decode(_ type: Int.Type, forKey key: Key) throws -> Int {
        ...
        guard let result = try decoder.unbox(value, as: Int.self) else {
            throw _valueNotFound(type)
        }
        return result
    }
    ...
    public func decode(_ type: Int8.Type, forKey key: Key) throws -> Int8 {
        ...
        guard let result = try decoder.unbox(value, as: Int8.self) else {
            throw _valueNotFound(type)
        }
        return result
    }
    ...
    public func decode(_ type: Int16.Type, forKey key: Key) throws -> Int16 {
        ...
        guard let result = try decoder.unbox(value, as: Int16.self) else {
            throw _valueNotFound(type)
        }
        return result
    }
    ...
    public func decode(_ type: Int32.Type, forKey key: Key) throws -> Int32 {
        ...
        guard let result = try decoder.unbox(value, as: Int32.self) else {
            throw _valueNotFound(type)
        }
        return result
    }
    ...
    public func decode(_ type: Int64.Type, forKey key: Key) throws -> Int64 {
        ...
        guard let result = try decoder.unbox(value, as: Int64.self) else {
            throw _valueNotFound(type)
        }
        return result
    }
    ...
    public func decode(_ type: UInt.Type, forKey key: Key) throws -> UInt {
        ...
        guard let result = try decoder.unbox(value, as: UInt.self) else {
            throw _valueNotFound(type)
        }
        return result
    }
    ...
    public func decode(_ type: UInt8.Type, forKey key: Key) throws -> UInt8 {
        ...
        guard let result = try decoder.unbox(value, as: UInt8.self) else {
            throw _valueNotFound(type)
        }
        return result
    }
    ...
    public func decode(_ type: UInt16.Type, forKey key: Key) throws -> UInt16 {
        ...
        guard let result = try decoder.unbox(value, as: UInt16.self) else {
            throw _valueNotFound(type)
        }
        return result
    }
    ...
    public func decode(_ type: UInt32.Type, forKey key: Key) throws -> UInt32 {
        ...
        guard let result = try decoder.unbox(value, as: UInt32.self) else {
            throw _valueNotFound(type)
        }
        return result
    }
    ...
    public func decode(_ type: UInt64.Type, forKey key: Key) throws -> UInt64 {
        ...
        guard let result = try decoder.unbox(value, as: UInt64.self) else {
            throw _valueNotFound(type)
        }
        return result
    }
    ...
    public func nestedContainer<NestedKey: CodingKey>(keyedBy _: NestedKey.Type, forKey _: Key) throws -> KeyedDecodingContainer<NestedKey> {
        // Not supported
...
extension _CSVRowDecoder: SingleValueDecodingContainer {
    ...
    private func expectNonNull(_ type: Any.Type) throws {
        guard !decodeNil() else {
            ...
            throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath: codingPath, debugDescription: description))
        }
    }
...
extension _CSVRowDecoder {
    ...
    fileprivate func unbox(_ value: String, as type: Bool.Type) throws -> Bool? {
        ...
        switch options.boolDecodingStrategy {
        case .default:
            guard let bool = Bool(value) else {
                throw _typeMismatch(at: codingPath, expectation: type, reality: value)
            }
            return bool
    ...
    fileprivate func unbox(_ value: String, as _: String.Type) throws -> String? {
        if value.isEmpty { return nil }

        return value
    }

    private func unbox(_ value: String, as _: Date.Type) throws -> Date? {
        if value.isEmpty { return nil }
    ...
    private func unbox(_ value: String, as _: Data.Type) throws -> Data? {
        if value.isEmpty { return nil }
